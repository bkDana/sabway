...
public class MenuService {
	...
	//메뉴목록 페이징
	public ApplyPageData menuListPaging(int currentPage1){
		String pageNavi = "";
		int numPerPage = 10;
		int pageNaviSize = 5;
		int totalCount = menudao.menuListTotalCount();
		int totalPage = (totalCount%numPerPage==0)?(totalCount/numPerPage):(totalCount/numPerPage)+1;
		int start = (currentPage1-1)*numPerPage+1;
		int end = currentPage1*numPerPage;
		ApplyMenuPageBound pb = new ApplyMenuPageBound(start,end,null);
		...
		return new CusOrderPageData(list,pageNavi,null);
	}
}
